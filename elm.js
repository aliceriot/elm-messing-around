...
Elm.Main.make = function (_elm) {
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var items = _U.list(["grapes","soda","beverages","milk","eggs","friendship"]);
   var listItem = function (itemName) {    return A2($Html.li,_U.list([]),_U.list([$Html.text(itemName)]));};
   var listItems = function (list) {    return A2($List.map,function (t) {    return A2($Html.li,_U.list([]),_U.list([$Html.text(t)]));},list);};
   var main = A2($Html.ul,_U.list([$Html$Attributes.$class("grocery-list")]),listItems(items));
   return _elm.Main.values = {_op: _op,listItems: listItems,listItem: listItem,items: items,main: main};
};
